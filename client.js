...
const main = () => {
	...
	const processCommand = async (command) => {
		...
		switch (command) {
			case 'get': {
				await get(args[1]).then((output) => console.log(output)).catch((error) => console.log(error));
				break;
			}
			case 'set': {
				await set(args[1], args[2], args.slice(3)).then((output) => console.log(output)).catch((error) => console.log(error));
				break;
			}
			case 'del': {
				await del(args.slice(1)).then((output) => console.log(output)).catch((error) => console.log(error));
				break;
			}
			case 'keys': {
				await keys('').then((output) => console.log(output)).catch((error) => console.log(error));
				break;
			}
			default: {
